...
class TestAddMember:
    ...
    def test_add_member(self):
        ...
        res = self.main.goto_main().goto_add_member().add_member().get_member()
        assert '松' in res

    @pytest.mark.skip
    def test_add_member_by_contact(self):
        # 1.跳转到通讯录  2.跳转添加成员页面  3. 添加成员 4.自动跳转到通讯录页面
        res = self.main.goto_main().goto_contact().goto_add_member().add_member().get_member()
        assert '松' in res
    ...
    def test_add_member_fail(self, accid, phone, expect_res):
        res = self.main.goto_main().goto_add_member().add_member_fail(accid, phone)
        assert expect_res in res

    @pytest.mark.skip
    def test_add_branch(self):
        self.main.goto_main().goto_contact().goto_add_branch()

    def teardown(self):
        self.main.goto_main().quit()
